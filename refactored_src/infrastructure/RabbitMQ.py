...
class RabbitMQ:
    ...
    def remove_queue(self) -> None:
        ...
        try:
            ...
            leftovers = []
            while True:
                method_frame, _, body = self.channel.basic_get(queue=self.queue_name, auto_ack=True)  # TODO:[] IS this not dangerous? auto acking all? what if anything happens while here? lost tasks or?
                if not method_frame:
                    break
                try:
